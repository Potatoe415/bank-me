...
// Check if database is already fully initialized and migrated
let isFullyMigrated = false;
try {
  // If the last migration's column exists, the schema is current
  db.prepare("SELECT applied_rule_detail FROM transactions LIMIT 0").get();
...
if (!isFullyMigrated) {
  ...
  const needsSourceUpdate = db.prepare("SELECT 1 FROM transactions WHERE source IS NULL OR source = '' LIMIT 1").get();
  if (needsSourceUpdate) {
    db.exec("UPDATE transactions SET source = 'api_enablebanking' WHERE source IS NULL OR source = ''");
  }

  const needsIsDeletedUpdate = db.prepare("SELECT 1 FROM transactions WHERE is_deleted IS NULL LIMIT 1").get();
  if (needsIsDeletedUpdate) {
    db.exec("UPDATE transactions SET is_deleted = 0 WHERE is_deleted IS NULL");
  }

  const needsIsManualUpdate = db.prepare("SELECT 1 FROM transactions WHERE category_is_manual IS NULL LIMIT 1").get();
  if (needsIsManualUpdate) {
  ...
  const needsReviewStatusUpdate = db.prepare("SELECT 1 FROM transactions WHERE review_status IN ('auto', 'manual') LIMIT 1").get();
  if (needsReviewStatusUpdate) {
    db.exec(`
  ...
  const needsManualBackfill = db.prepare(`
  ...
  db.transaction(() => {
    const rows = db
      .prepare("SELECT DISTINCT category_path FROM transactions WHERE category_path != 'uncategorized' AND cashflow_type = 'expense' AND behavior_bucket = 'variable'")
...
// Seed taxonomy_entries with the canonical flat paths if not already populated
const currentTaxonomyCount = (db.prepare("SELECT COUNT(*) as count FROM taxonomy_entries").get() as { count: number }).count;
if (currentTaxonomyCount < CATEGORY_PATHS.length) {
