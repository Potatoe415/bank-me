...
const ACTIONABLE_COUNTERPART_MAX_SPEND = 150;
const MONTHLY_BUDGET_LIMIT = 1200;
...
function makeActionableCategoryLabel(path: string): string {
  if (!path || path === "uncategorized") return "Uncategorized";
  // Break out subscription leaf names directly
  if (path.startsWith("fixed.subscriptions.")) {
    const leaf = path.slice("fixed.subscriptions.".length);
    return leaf.charAt(0).toUpperCase() + leaf.slice(1).replace(/_/g, " ") + " subscriptions";
  }
  ...
  return [...rowsByCurrency.entries()]
    .map(([currency, currencyRows]) => {
      ...
      for (const row of currencyRows) {
        ...
        const label = makeActionableCategoryLabel(path);
        const categoryEntry = categoryTotals.get(label) ?? { amount: 0, count: 0 };
        categoryEntry.amount += spend;
        categoryEntry.count += 1;
        categoryTotals.set(label, categoryEntry);
        ...
        // Exclude travel from counterpart analysis
        if (path !== "irregular.travel" && spend <= ACTIONABLE_COUNTERPART_MAX_SPEND) {
...
export default async function GraphicsActionablePage({ searchParams }: PageProps) {
  ...
  const title = selectedBank ? `${selectedBank.name} actionable drilldown` : "Actionable drilldown";
  ...
  const description = selectedBank
    ? `Leak-focused spending analysis for ${selectedBank.name} over ${periodDescription}.`
    : `Leak-focused spending analysis across all banks over ${periodDescription}.`;
      ...
      <div className="mb-8 flex flex-col gap-5">
        ...
        <div className="flex flex-col gap-2">
          ...
          <p className="max-w-3xl text-sm text-gray-500">
            {description} This view keeps the same outgoing-only scope, then zooms in on routine spending that is easier to trim.
          </p>
        </div>
        ...
        <div className="flex flex-col gap-4 rounded-2xl border border-gray-200 bg-white p-4">
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              <Link
                href={buildActionableHref("all", period.value)}
              ...
              {bankOptions.map((bank) => (
                <Link
                  key={bank.id}
                  href={buildActionableHref(bank.id, period.value)}
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {GRAPHICS_PERIOD_OPTIONS.map((option) => (
                <Link
                  key={option.value}
                  href={buildActionableHref(bankId, option.value)}
                  className={`rounded-full px-3 py-1.5 text-sm transition-colors ${
        ...
        <div className="space-y-8">
          {analytics.map((currencyData) => {
            ...
            const chartMax = Math.max(
              ...currencyData.monthlyTrend.map((month) => month.amount),
              MONTHLY_BUDGET_LIMIT,
              0
            );
            const referenceLineOffset =
              chartMax > 0 ? Math.min((MONTHLY_BUDGET_LIMIT / chartMax) * 100, 100) : 0;
              ...
              <section key={currencyData.currency} className="space-y-5">
                <div className="flex flex-wrap items-end justify-between gap-3">
                  ...
                  <p className="max-w-xl text-sm text-gray-500">
                    {biggestCategory
                      ? `${biggestCategory.name} is the biggest actionable bucket at ${Math.round(biggestCategory.share * 100)}% of spend.`
                      : "No category split available yet."}{" "}
                    {biggestCounterpart
                      ? `${biggestCounterpart.name} appears most often among smaller recurring payments.`
                      : ""}
                  </p>
                </div>
                ...
                <div className="grid gap-4 md:grid-cols-3">
                  ...
                  <div className="rounded-2xl border border-gray-200 bg-white p-5">
                    ...
                    <p className="mt-2 text-sm text-gray-500">
                      Smaller recurring payments tend to hide below this average.
                    </p>
                  </div>
                ...
                <div className="grid gap-4 xl:grid-cols-[1.35fr_1fr]">
                    ...
                    <div className="mb-5 flex items-center justify-between gap-3">
                      <div>
                        ...
                        <p className="mt-1 text-sm text-gray-500">
                          Subscriptions are broken down directly by sub-path.
                        </p>
                      </div>
                    ...
                    <div className="space-y-3">
                      {currencyData.topCategories.map((category) => {
                        return (
                          <div
                            key={category.name}
                            className="block rounded-xl border border-transparent p-2"
                          >
                  ...
                  <div className="rounded-2xl border border-gray-200 bg-white p-5">
                    ...
                    <p className="mt-1 text-sm text-gray-500">
                      Last six visible months with a strict monthly cap reference at {fmtMoney(MONTHLY_BUDGET_LIMIT, currencyData.currency)}.
                    </p>

                    <div className="mt-8 relative min-h-56">
                      <div
                        className="pointer-events-none absolute inset-x-0 border-t border-dashed border-rose-400"
                        style={{ bottom: `${referenceLineOffset}%` }}
                      />
                      <div
                        className="pointer-events-none absolute right-0 -translate-y-1/2 rounded-full bg-rose-50 px-2 py-0.5 text-[11px] font-medium text-rose-700"
                        style={{ bottom: `${referenceLineOffset}%` }}
                      >
                        Limit {fmtMoney(MONTHLY_BUDGET_LIMIT, currencyData.currency)}
                      </div>

                      <div className="flex min-h-56 items-end gap-3">
                        {currencyData.monthlyTrend.map((month) => {
                          const height = chartMax > 0 ? (month.amount / chartMax) * 100 : 0;
                ...
                <div className="rounded-2xl border border-gray-200 bg-white p-5">
                  <div className="mb-5 flex items-center justify-between gap-3">
                    <div>
                      ...
                      <p className="mt-1 text-sm text-gray-500">
                        Recurring small-to-medium outgoing payments, excluding travel and one-off spikes above {fmtMoney(ACTIONABLE_COUNTERPART_MAX_SPEND, currencyData.currency)}.
                      </p>
                    </div>
                    <span className="text-xs text-gray-400">
                      Ranked by frequency first
                    </span>
                  </div>

                  {currencyData.topCounterparts.length === 0 ? (
                    <div className="rounded-xl border border-dashed border-gray-200 bg-gray-50/70 p-4 text-sm text-gray-500">
                      No recurring small-to-medium counterparts matched this filter yet.
                    </div>
                  ) : (
